...
mkdir -p ${OUTDIR}/rootfs/{bin,dev,etc,home,lib,lib64,proc,root,run,sbin,sys,tmp,usr/{bin,sbin},var}
...
${CROSS_COMPILE}readelf -d ${OUTDIR}/rootfs/bin/busybox | grep "Shared library" | \
    awk -F'[][]' '{print $2}' | \
    while read lib;\
        do \
            find ${SYSROOT} -name "$lib" -exec cp -av {} ${OUTDIR}/rootfs/lib64 \; ; \
        done \
